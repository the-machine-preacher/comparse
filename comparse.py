...
class comparse:
    ...
    def __repr__(self):
        return "Command parsing for this program was done using COMPARSE: a flexible commandline parsing module. Designed to pick out ATTRIBUTES and assign VALUES to them from a message containing many un-formatted attributes/variables.\n"
        ...
        def attribute_match(string, match):
            ...
            for word in string.split(): 
                if match == word: 
                    ...
                    return match
            #Finds the closest match and conforms the original message to fit the attribute. Possibilities that don’t score at least that 0.7 similar to word are ignored.
            if len(match) > 4: #Only activate difflib if the match string is greater than 4 characters long.
                if difflib.get_close_matches(match, string.split(),3,0.7): 
                    self.message = self.message.replace(difflib.get_close_matches(match, string.split())[0]+' ', match+'  ') #This prevents adjacent matches from 'grabbing' each other.
                    return match
                    ...
                    if type(element) is str:
                        for sub_element in element.split(" "): 
                            try: 
                                ...
                                break #Find the first interger then break the loop immediately, don't look for anything else!
                            except: pass 
                    ...
                    if type(element) is str:
                        for sub_element in element.split(" "): 
                            try: 
                                ...
                                break #Find the first floating-point number then break the loop immediately, don't look for anything else!
                            except: pass 
    ...
    def find_unit_value(self, message, variable, unit, default=0, help_txt="Specify a variable and its unit of measure"):
